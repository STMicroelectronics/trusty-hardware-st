 ...
 *
 * @retval: Return NO_ERROR on success or error return code.
 *
...
int mbox_register_chan(mbox_callback_t rcv_cb,
      ...
      unsigned int chan_id, struct mbox_chan **chan);
...
int mbox_dt_register_chan(mbox_callback_t rcv_cb,
    mbox_callback_t txc_cb, void *cookie,
    const struct mbox_fdt *mboxes_fdt, size_t nmboxes, struct mbox_chan **chan);
 ...
 * @chan: Pointer to mbox chan pointer handle.
 *
 * @retval: Return NO_ERROR on success or appropriate error code.
 *
...
int mbox_dt_register_chan_by_name(mbox_callback_t rcv_cb,
    mbox_callback_t txc_cb, void *cookie, const struct mbox_fdt *mboxes_fdt,
    size_t nmboxes, const char *name, struct mbox_chan **chan);
 ...
 *
 * @retval: ERR_BUSY If the remote hasn't yet read the last data sent.
 * @retval: ERR_TOO_BIG If size exceeds the max data size
 *	    supported by the device.
 * @retval: ERR_INVALID_ARGS If there was a bad parameter, such as:
 ...
 *
 * @retval: ERR_NO_MSG when no data available.
 * @retval: ERR_INVALID_ARGS If error on parameters.
 * @retval: ERR_TOO_BIG If size exceeds the max
...
#define MBOX_RX_NOTIF_CAP  (1 << MBOX_RX_NOTIF)
#define MBOX_TX_NOTIF_CAP  (1 << MBOX_TX_NOTIF)
...
struct mbox_ops {
     ...
     *
     * @retval: ERR_INVALID_ARGS for incorrect param.
     * @retval: ERR_BUSY if remote not ready for reception.
     * @retval: NO_ERROR if send is success.
     */
    int (*send)(const struct mbox_desc *desc,
        unsigned int id, const void *data, size_t size);
     ...
     */
    int (*register_callback)(const struct mbox_desc *desc,
     ...
     * @retval: Return NO_ERROR on success or error return code.
     */
    int (*channel_interrupt)(const struct mbox_desc *desc,
     ...
     * @retval: Return NO_ERROR on success or error return code.
     */
    int (*complete)(const struct mbox_desc *desc, unsigned int id);
